...
class BenchmarkEnv:
    ...
    @property
    def cache_dir(self):
        return os.path.expanduser(os.environ.get("ORCAR_CACHE_DIR", "~/.orcar"))
    ...
    def create_conda_env(self, inst: Dict[str, Any]) -> None:
        ...
        if install_configs.get("install"):
            ...
            try:
                ...
                if " -e " not in install_cmd:
                    raise
                fallback_install_cmd = install_cmd.replace(" -e ", " ", 1)
                logger.warning(
                    "Editable install failed; retrying non-editable install: %s",
                    fallback_install_cmd,
                )
                self.run_with_handle(
                    fallback_install_cmd,
                    err_msg="Fallback install command failed to execute successfully",
                    timeout=LONG_TIMEOUT,
                    output_log=True,
                )
